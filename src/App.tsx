...
export function App() {
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      ...
      // 배포 초기 상태: 이름이 없으면 모달 열기
      if (!parsedProfile.name) setIsProfileModalOpen(true);
    } else {
      setIsProfileModalOpen(true); // 완전 처음 시작 시 모달 열기
    }
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/40 backdrop-blur-sm p-6">
          <div className="bg-white rounded-[32px] p-6 w-full max-w-sm shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              ...
              {!childProfile.name && <span className="text-[10px] font-bold text-indigo-500">반가워요!</span>}
          ...
          <main className="max-w-4xl mx-auto p-4 md:p-6 space-y-6 flex-1 w-full pb-20">
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {missions.length === 0 ? (
                  <p className="text-xs text-slate-400 p-4 text-center border-2 border-dashed rounded-2xl w-full col-span-2">아직 등록된 미션이 없습니다.</p>
                ) : missions.map(m => (
